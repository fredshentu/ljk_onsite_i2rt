...
class QuestReader:
    ...
    _low_pass_filter = LowPassFilter(
        alpha=0.3, order=1
    )  # if enable_low_pass_filter is True, the filter will be applied on Trigger
        ...
        try:
            if processed_frame is None:
                ...
                for k, v in parsed_data.items():
                    ...
                    elif "Trigger" in k and self.enable_low_pass_filter:
                        processed_data[k] = self._low_pass_filter.filter({k: v})[k]
                    else:
                        processed_data[k] = v
